...
First public alpha release of Mithril Veil — self-hosted Russian PII anonymization for safer AI workflows.
...
- **CLI `anonymize-dir` (Slice 10)** — recursive batch processing for `.txt`, `.md`, `.markdown`, `.docx`, `.odt`, `.rtf`, and text-based `.pdf`; outputs `*.anonymized.txt` under `--output-dir`; aggregate safe JSON report; `replace`/`redact` only (no batch mapping or `pseudonymize`).
- **`pseudonymize` mode (Slice 8)** — deterministic typed placeholders with optional reversible mapping kept in memory during a run; API responses never include mapping payloads or raw detected values.
- **Encrypted local mapping files (Slice 8)** — CLI `--mapping-output` (`.json.enc` only) with passphrase from `MITHRIL_VEIL_MAPPING_PASSPHRASE` (override via `--mapping-passphrase-env`); refuse overwrite unless `--force`.
- **Safe report mapping metadata (Slice 8)** — reports may include only `mapping.written` and `mapping.encrypted` flags, never placeholder-to-original entries.
...
- **Security checklist (Slice 9)** — reversible mapping / pseudonymization section and pre-release checks in [docs/security_checklist.md](docs/security_checklist.md).
- **Slice 8.1 mapping hardening** — removed unused `app/security/mapping_io.py`; single canonical path `app/security/encrypted_mapping.py`; CLI refuses mapping path equal to input/output/report; expanded security regression tests for pseudonymize/mapping errors.

### Slices 1–7 (initial delivery)
